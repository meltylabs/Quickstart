(() => {
  ...
  const CART_KEY = "spl-gold-demo-cart-v1";
  const RECENT_KEY = "spl-gold-recent-products-v1";
  const ORDER_KEY = "spl-gold-demo-orders-v1";
  const CHECKOUT_KEY = "spl-gold-checkout-draft-v1";
  ...
  const PROMO_CODES = {
    "SPL-FIRST": { label: "First order", type: "percent", value: 10 },
    "COLDCHAIN": { label: "Free cold chain", type: "shipping", value: 100 },
  };

  /* Fictional analytical fixtures for the interactive demo. They are marked as
     sample data everywhere they appear and are never presented as real records. */
  const TESTING = {
    "retatrutide":            { lot: "SPL-RT15", date: "2026-07-26", identity: ["LC-MS", "Sample conforms"], content: ["Gravimetric", "15.08 mg"], purity: ["RP-HPLC", "99.61%"] },
    "semaglutide":            { lot: "SPL-SM10", date: "2026-07-24", identity: ["LC-MS", "Sample conforms"], content: ["Gravimetric", "10.04 mg"], purity: ["RP-HPLC", "99.48%"] },
    "tirzepatide":            { lot: "SPL-TZ10", date: "2026-07-22", identity: ["LC-MS", "Sample conforms"], content: ["Gravimetric", "10.02 mg"], purity: ["RP-HPLC", "99.42%"] },
    "cagrilintide":           { lot: "SPL-CG10", date: "2026-07-21", identity: ["LC-MS", "Sample conforms"], content: ["Gravimetric", "9.96 mg"], purity: ["RP-HPLC", "98.92%"] },
    "bpc-157":                { lot: "SPL-BP10", date: "2026-07-23", identity: ["LC-MS", "Sample conforms"], content: ["Gravimetric", "10.01 mg"], purity: ["RP-HPLC", "99.18%"] },
    "tb-500":                 { lot: "SPL-TB10", date: "2026-07-20", identity: ["LC-MS", "Sample conforms"], content: ["Gravimetric", "10.06 mg"], purity: ["RP-HPLC", "99.06%"] },
    "ipamorelin":             { lot: "SPL-IP05", date: "2026-07-19", identity: ["LC-MS", "Sample conforms"], content: ["Gravimetric", "5.02 mg"], purity: ["RP-HPLC", "99.22%"] },
    "cjc-1295":               { lot: "SPL-CJ05", date: "2026-07-18", identity: ["LC-MS", "Sample conforms"], content: ["Gravimetric", "5.01 mg"], purity: ["RP-HPLC", "98.96%"] },
    "bpc-tb-blend":           { lot: "SPL-BT10", date: "2026-07-25", identity: ["LC-MS", "Both samples conform"], content: ["Gravimetric", "5.03 + 5.01 mg"], purity: ["RP-HPLC", "99.34%"] },
    "cjc-ipamorelin-blend":   { lot: "SPL-CI10", date: "2026-07-25", identity: ["LC-MS", "Both samples conform"], content: ["Gravimetric", "5.01 + 5.02 mg"], purity: ["RP-HPLC", "99.12%"] },
  };
  ...
  /* Demo inventory is canonical data, not markup, so cards, PDPs, cart limits,
     and checkout all agree. None of these counts represent real inventory. */
  ...
  function cartComponentDemand(lines) {
    const demand = new Map();
    lines.forEach((line) => {
      const item = cartItemData(line.slug);
      const components = item?.productSlugs || (item ? [item.slug] : []);
      components.forEach((slug) => {
        demand.set(slug, (demand.get(slug) || 0) + line.quantity);
      });
    });
    return demand;
  }

  function aggregateInventoryShortages(lines) {
    const demand = cartComponentDemand(lines);
    return [...demand.entries()]
      .filter(([slug, units]) => units > stockFor(slug))
      .map(([slug, units]) => {
        const product = PRODUCTS.find((entry) => entry.slug === slug);
        const contributorSlugs = lines
          .filter((line) => {
            const item = cartItemData(line.slug);
            return (item?.productSlugs || [line.slug]).includes(slug);
          })
          .map((line) => line.slug);
        return {
          slug,
  ...
  function cartLineIssue(line) {
    ...
    const aggregate = aggregateInventoryShortages(cart).find((issue) =>
      issue.contributorSlugs.includes(line.slug),
    );
    if (aggregate) {
      return {
        type: "aggregate",
  ...
  function cartLineCeiling(slug) {
    ...
    if (!stock.sellable || !item) return 0;
    const otherLines = cart.filter((line) => line.slug !== slug);
    const otherDemand = cartComponentDemand(otherLines);
    const components = item.productSlugs || [item.slug];
    const componentCeiling = Math.min(
      ...components.map((componentSlug) =>
        Math.max(0, stockFor(componentSlug) - (otherDemand.get(componentSlug) || 0)),
      ),
    );
    const ownCeiling = item.productSlugs ? stockFor(item.slug) : componentCeiling;
    return Math.max(
      0,
      Math.min(MAX_QUANTITY, stock.units, componentCeiling, ownCeiling),
    );
  }
  ...
  const PRODUCTS = [
    {
      ...
      facts: ["15 mg labeled amount", "Single-vial format", "Identity, content, purity reported"],
      overview:
  ...
  function track(event, properties = {}) {
    ...
    window.dispatchEvent(new CustomEvent("standardpeptidelabs:analytics", { detail: payload }));
  }

  window.StandardPeptideLabsAnalytics = {
  ...
  function header() {
    ...
    return `
      ...
      <div class="demo-banner" role="note">
        ...
        <span>Checkout and payment processing are simulated. No charge or order is created.</span>
      </div>
      <div class="announcement" role="note">
        <p>
          <span>Complimentary priority shipping on demo orders over $250</span>
        ...
        <div class="shell">
          <div class="footer-grid">
            <div class="footer-brand">
              ...
              <p>A complete browser-local commerce demo where every product, lot, cart state, and checkout step can be evaluated.</p>
            </div>
            ...
            <div class="footer-column">
              <h2>Company</h2>
              <a href="${path("company.html")}">Our approach</a>
              <a href="${path("account.html")}">Demo account</a>
              <a href="${path("faq.html")}">FAQ</a>
              <a href="${path("policies.html")}">Privacy & terms</a>
            </div>
          </div>
          <div class="footer-bottom">
            <div>© 2026 Standard Peptide Labs · Interactive demo</div>
            <span>Fictional catalog and testing data. No payment, order, or shipment is created.</span>
          </div>
        </div>
      </footer>
    `;
  ...
  function staticSearchDialog() {
    return `
      <dialog class="static-search-dialog" data-static-search aria-labelledby="static-search-title">
        <div class="static-search-head">
          <span id="static-search-title">Search the catalog</span>
          <button type="button" data-static-search-close aria-label="Close search">×</button>
        </div>
        <div class="static-search-body">
          <div class="search-wrap">
            ...
            <input
              ...
              type="search"
              role="combobox"
              placeholder="Search molecule, code, category, or strength"
              autocomplete="off"
        ...
        <div class="static-search-footer">
          <span>Try “Retatrutide”, “SPL-001”, or “metabolic”</span>
          <a href="${path("catalog.html")}">Browse all products</a>
        </div>
      </dialog>
    `;
  }
  ...
  function productImage(product, eager = false) {
    return `
      <img
        ...
        alt="Standard Peptide Labs vial for ${escapeHtml(product.name)}"
        width="1024"
        height="1024"
      ...
      <section class="home-hero">
        <div class="shell hero-grid">
          ...
          <div class="hero-visual">
            <img src="${path("../assets/datum-hero-vials.webp")}" alt="An Standard Peptide Labs amber peptide vial with a gold cap resting on a travertine ledge in warm window light" width="1536" height="1024" fetchpriority="high">
          </div>
        </div>
      </section>
  ...
  function testingPage() {
    return `
      <section class="page-hero">
        <div class="shell">
          <span class="sample-watermark">Fictional sample records</span>
          <p class="eyebrow">Testing & COAs</p>
          <h1>Testing status belongs beside the product.</h1>
          <p>Search an Standard Peptide Labs product name or code to see the testing status for its current lot. Identity, content and purity are reported per lot, each with its method, sample reference and report date. Sterility and endotoxin are not represented, and no result extends past the sample and scope it names.</p>
          <form class="testing-lookup" data-testing-lookup>
            <label class="sr-only" for="testing-search">Search product name or code</label>
            <input id="testing-search" name="query" type="search" placeholder="Try SPL-001 or Retatrutide" autocomplete="off">
        ...
        <div class="shell detail-grid">
          ...
          <div>
            ...
            <p class="testing-verified">${icons.shield}<span><strong>Third-party verified.</strong> Independent US laboratory.</span></p>
              <p class="testing-attribution">Analysis performed by an independent contract laboratory in the United States. Methods, sample reference, lot and report date are shown per entry. Standard Peptide Labs does not perform its own release testing.</p>
          </div>
        </div>
      </section>
      <section class="section section-surface" id="lot-records">
          ...
          <div>
            ${accordion([
              ...
              [
                ...
                "No. Standard Peptide Labs does not present human-use, safety, efficacy, clinical, administration, or preparation information.",
              ],
              [
    ...
    return `
      ...
      <section class="section section-surface">
        <div class="shell detail-grid">
          ...
          <div>
            ${accordion([
              ["Is Standard Peptide Labs a real company?", "No. It is a fictional, non-operational storefront concept. Nothing is stocked, sold, shipped, tested, or transacted."],
  ...
  function faqPage() {
    return `
      ...
      <section class="section">
        <div class="shell detail-grid">
          <div>
            <p class="eyebrow">Standard Peptide Labs FAQ</p>
            <h2>The complete store boundary.</h2>
          </div>
          <div>
            ${accordion([
              ["Can I place an order?", "You can complete the checkout and receive an order reference, which is recorded in this browser. Because Standard Peptide Labs is a fictional concept storefront, nothing is charged, stocked, shipped, or fulfilled."],
              ...
              ["How does search work?", "Search by peptide name, category, blend name, or catalog code such as SPL-001. Keyboard users can move through predictive results with the arrow keys."],
              ["Why are there no customer reviews?", "There are no customers or order history. Standard Peptide Labs does not fabricate ratings, testimonials, or purchase activity."],
  ...
  function aboutPage() {
    return `
      <section class="page-hero">
        <div class="shell">
          <p class="eyebrow">About Standard Peptide Labs</p>
          <h1>A peptide store designed around finding and deciding.</h1>
          <p>Standard Peptide Labs is designed around a simple idea: product information should be easy to scan, and missing evidence should be impossible to miss.</p>
        </div>
      </section>
      <section class="section">
        <div class="shell detail-grid">
          ...
          <div>
            <p class="article-lede">The strongest specialist stores help people find the right entry quickly, compare it in place, and understand the next action. Standard Peptide Labs applies that discipline without inventing operations or proof.</p>
      ...
      <section class="section">
        <div class="shell detail-grid">
          ...
          <div>
            ...
            <div class="spec-table">
              <div class="spec-row"><strong>Operating entity</strong><span>Standard Peptide Labs LLC, an Oregon limited liability company</span></div>
              ...
              <div class="spec-row"><strong>Analytical work</strong><span>An independent US contract laboratory. Standard Peptide Labs runs no release testing of its own and names the method, sample and date on every lot it reports</span></div>
              <div class="spec-row"><strong>Support</strong><span><a class="text-link" href="mailto:support@standardpeptidelabs.example">support@standardpeptidelabs.example</a>, Monday to Friday, 9:00 to 17:00 Pacific</span></div>
      ...
      <section class="section section-surface" id="contact">
        <div class="shell detail-grid">
          <div>
            <p class="eyebrow">Demo contact</p>
            <h2>Test the support handoff.</h2>
            <p>This form exercises the success state only. Its contents are not saved or transmitted.</p>
          </div>
          <div>
            <form class="demo-contact-form" data-demo-contact>
              <div class="field-grid">
                ${checkoutField("contactName", "Name", { autocomplete: "name", width: "half" })}
                ${checkoutField("contactEmail", "Email", { type: "email", autocomplete: "email", width: "half" })}
                ${checkoutField("contactOrder", "Demo order reference", { required: false, placeholder: "SPL-20260726-1234" })}
              ...
              <button class="button button-primary" type="submit">Record demo message</button>
              <p class="step-hint">Nothing leaves this browser. Do not enter sensitive information.</p>
            </form>
            <div class="contact-success" data-contact-success role="status" hidden>
              <span class="sample-watermark">Demo only</span>
              <h3>Message flow complete.</h3>
              <p>A real support system would create a case and send a confirmation. This demo intentionally did neither.</p>
              <button class="button button-secondary" type="button" data-contact-reset>Send another demo message</button>
            </div>
          </div>
        </div>
      </section>
    ...
    return `
      <section class="page-hero">
        <div class="shell">
          ...
          <p>Standard Peptide Labs has no company operations, products, inventory, customers, testing, fulfillment, payments, or ordering capability.</p>
        </div>
      </section>
      <article class="article-shell">
        ...
        <ul>
          ...
          <li>It does not transmit payment, customer-account, address, phone, or submitted email data. Checkout fields and demo orders can be retained only in this browser so the workflow can be tested.</li>
        ...
        <h2 id="privacy">Demo privacy</h2>
        <p>Cart contents, checkout contact and shipping fields, promotion choice, and up to ten demo orders use this origin's browser storage. Sandbox card number, expiry, and CVC values are never written to storage and no storefront data is sent over the network.</p>
        <p>Use <a class="text-link" href="${path("account.html")}">Demo account</a> to inspect or erase the saved state. Your browser's site-data controls can erase it as well.</p>
        <h2 id="terms">Demo terms</h2>
        <p>Completing checkout creates a browser-local demonstration record only. It does not create a sale, payment obligation, reservation, customer account, shipment, support entitlement, or agreement with a real business.</p>
        ...
        <p><a class="text-link" href="mailto:support@standardpeptidelabs.example">Email support@standardpeptidelabs.example</a></p>
    ...
    return `
      <section class="page-hero">
        <div class="shell">
          ...
          <p>Standard Peptide Labs presents research material for laboratory and research settings only. This page states the demo checkout boundary and where the fictional concept would ship.</p>
        </div>
      </section>
      <article class="article-shell">
        <h2>Who may order</h2>
        ...
        <h2>What can enter the demo checkout</h2>
        <p>All catalog entries can be added to this browser-local demo cart while their fictional inventory count is above zero. That interaction demonstrates availability, quantity limits, totals, and order-state handling; it is not an offer or sale of real material.</p>
        ...
        <p>Standard Peptide Labs is a fictional concept storefront. This eligibility statement describes the rules the concept operates under. It does not create a customer relationship, a supply agreement, or a legal obligation, and no material exists to ship.</p>
          ...
          <div>
            ${(() => {
              ...
              return accordion([
                ...
                ["Where is administration information?", "It is intentionally absent. Standard Peptide Labs does not provide dosing, preparation, administration, human-use, safety, efficacy, or outcome information."],
              ]);
            })()}
          </div>
        </div>
  ...
  function articlePage(slug) {
    ...
    return `
      ...
      <section class="section section-surface">
        <div class="shell">
          <div class="section-head"><div><p class="eyebrow">More from Standard Peptide Labs</p><h2>Related guides</h2></div></div>
          <div class="education-grid">
  ...
  function lotRecordPage() {
    ...
    if (!product) {
      ...
      return `
        ...
        <section class="section">
          <div class="shell">
            ${lotRecordIndex()}
            <p class="testing-attribution">Analysis performed by an independent contract laboratory in the United States. Standard Peptide Labs does not perform its own release testing. No record on this site represents sterility or endotoxin.</p>
          </div>
        </section>
      `;
    }
    ...
    return `
      ...
      <section class="page-hero is-compact">
        <div class="shell">
          <span class="sample-watermark">Sample / demo record</span>
        ...
        <div class="shell lot-layout">
          <article class="lot-doc">
            <header class="lot-doc-head">
              <div>
                <p class="lot-doc-kicker">Sample / demo analytical record</p>
      ...
      <section class="section">
        <div class="shell">
          <div class="section-head">
            ...
            <p>Four research areas, and four catalog entries to start from. The header search also takes a molecule name or a catalog code such as SPL-002.</p>
          </div>
        ...
        return `
          <article class="cart-row" data-cart-row="${item.slug}">
            <a class="cart-row-image" href="${item.productSlugs ? path("bundles.html") : path(`peptides/${item.slug}.html`)}">
              <img src="${path(item.productSlugs ? "../assets/ovo-set-pair.webp" : "../assets/datum-product-vial.webp")}" alt="${escapeHtml(item.name)}" width="120" height="135">
            </a>
            <div class="cart-row-body">
              ...
              ${cartLineAtCap(line)
                ? cartLineCeiling(line.slug) > 0
                  ? `<p class="cart-row-cap">Maximum ${cartLineCeiling(line.slug)} per order at current shared stock.</p>`
                  : `<p class="cart-row-cap">Shared inventory is fully allocated by other cart lines.</p>`
                : ""}
            </div>
          </article>
        `;
        ...
        <div class="shell cart-layout">
          <div class="cart-main">
            ${(() => {
              ...
              const unsellable = issues.filter((issue) =>
                ["restricted", "unavailable"].includes(issue.type),
              );
              const partial = issues.filter((issue) => issue.type === "partial");
              const aggregate = [
                ...new Map(
                  issues
                    .filter((issue) => issue.type === "aggregate")
                    .map((issue) => [issue.componentName, issue]),
                ).values(),
              ];
              const parts = [];
              ...
              aggregate.forEach((issue) => {
                parts.push(
                  `<span>Your individual items and sets require ${issue.demand} units of ${escapeHtml(issue.componentName)}, but only ${issue.units} are available. Lower either contributing quantity to continue.</span>`,
                );
              });
          ...
          <div class="cart-aside">
            ...
            <form class="promo-form" data-promo-form>
              ...
              <p class="promo-hint">Try <button type="button" class="link-button" data-fill-promo="SPL-FIRST">SPL-FIRST</button></p>
  ...
  const DEMO_PROCESSOR = "DemoPay sandbox";
  const DEMO_APPROVED_CARD = "4242424242424242";
  const DEMO_DECLINED_CARD = "4000000000000002";
  ...
  function validExpiry(value, now = new Date()) {
  ...
  function sandboxTransactionId() {
    ...
    return `dpy_test_${Date.now().toString(36)}_${[...random].map((part) => part.toString(36)).join("")}`;
  }

  /* This intentionally simulates a processor in memory. It performs no fetch,
     XHR, beacon, form post, or other network request. Raw payment fields never
     leave the checkout DOM and are never included in browser storage. */
  function authorizeDemoPayment(number) {
    ...
    return new Promise((resolve) => {
      window.setTimeout(() => {
        ...
        if (digits === DEMO_APPROVED_CARD) {
          resolve({ ...base, outcome: "approved" });
          return;
        }
        if (digits === DEMO_DECLINED_CARD) {
          resolve({
            ...base,
            outcome: "declined",
            message: "DemoPay declined this sandbox card. Use 4242 4242 4242 4242 to approve, then retry.",
          });
          return;
        }
        resolve({
          ...base,
          outcome: "declined",
          message: "DemoPay accepts only the two test card numbers shown above. No request was sent.",
        });
      }, 700);
    });
  }
  ...
  function checkoutPage() {
    ...
    if (blocked.length) {
      const aggregate = aggregateInventoryShortages(cart);
      const reason = aggregate.length
        ? aggregate
            .map(
              (issue) =>
                `${escapeHtml(issue.name)} demand is ${issue.demand} units across individual items and sets; ${issue.units} are available.`,
            )
            .join(" ")
        : `${blocked.map((line) => escapeHtml(cartItemData(line.slug) ? cartItemData(line.slug).name : line.slug)).join(", ")} cannot be sold at the saved quantities.`;
      return `
        <section class="page-hero">
          <div class="shell">
            <p class="eyebrow">Checkout</p>
            <h1>Your cart quantities need attention.</h1>
            <p>${reason} Return to the cart and lower or remove a contributing line to continue.</p>
            <div class="hero-actions">
        ...
        <div class="shell checkout-layout">
          <form class="checkout-main" data-checkout-form novalidate>
            ...
            <fieldset class="checkout-step" data-step="payment" hidden>
              ...
              <div class="field-grid">
                <div class="field is-half">
                  ...
                  <input id="cardNumber" name="cardNumber" type="text" inputmode="numeric" autocomplete="off" maxlength="23" value="4242 4242 4242 4242" aria-describedby="cardNumber-note cardNumber-error">
                  <p class="field-note" id="cardNumber-note">Sandbox values only. This value stays in memory for this page.</p>
                  <p class="field-error" id="cardNumber-error" data-error-for="cardNumber"></p>
                </div>
                <div class="field is-quarter">
                  <label for="cardExpiry">Expiry</label>
                  <input id="cardExpiry" name="cardExpiry" type="text" inputmode="numeric" autocomplete="off" maxlength="7" value="12 / 30" aria-describedby="cardExpiry-error">
                  <p class="field-error" id="cardExpiry-error" data-error-for="cardExpiry"></p>
                </div>
                <div class="field is-quarter">
                  <label for="cardCvc">CVC</label>
                  <input id="cardCvc" name="cardCvc" type="text" inputmode="numeric" autocomplete="off" maxlength="4" value="123" aria-describedby="cardCvc-error">
              ...
              <div class="step-actions">
                ...
                <a class="button button-secondary" href="${path("cart.html")}">Back to cart</a>
                <button class="button button-primary" type="button" data-next="payment">Authorize sandbox payment</button>
              </div>
            </fieldset>
            ...
            <fieldset class="checkout-step" data-step="review" hidden>
              ...
              <p class="field-error" data-order-error role="alert" aria-live="assertive"></p>
  ...
  function confirmationPage() {
    ...
    const order = requestedReference
      ? orders.find((entry) => entry.reference === requestedReference)
      : orders[0];
      ...
      .map(
        (line) => `
          <div class="summary-line">
            <img src="${path(line.isBundle ? "../assets/ovo-set-pair.webp" : "../assets/datum-product-vial.webp")}" alt="${escapeHtml(line.name)}" width="48" height="54">
            <div>
              <strong><a href="${line.isBundle ? path("bundles.html") : path(`testing.html?product=${encodeURIComponent(String(line.code || ""))}`)}">${escapeHtml(line.name)}</a></strong>
    ...
    const paymentSummary = payment
      ? `<strong>${escapeHtml(payment.processor || DEMO_PROCESSOR)}</strong><br>
         ${payment.outcome === "approved" ? "Approved sandbox authorization" : escapeHtml(payment.outcome || "Unknown outcome")}${Number.isFinite(Number(payment.authorizedAmount)) ? ` for ${money.format(Number(payment.authorizedAmount))}` : ""} · ${escapeHtml(payment.brand || "Card")} ending in ${escapeHtml(payment.last4 || "••••")}<br>
         <span class="lot-nb">Transaction ${escapeHtml(payment.transactionId || "not recorded")}</span><br>
         No charge was made. This sandbox authorization never left the browser.`
      : `No sandbox transaction was recorded for this legacy demo order.<br>No charge was made.`;
          ...
          <div class="confirmation-main">
            ...
            <div class="confirmation-details">
              ...
              <div class="detail-card">
                ...
                <p>Photograph the box, mailer, vial label and logger within 72 hours, then email <a style="text-decoration:underline;text-underline-offset:2px" href="mailto:support@standardpeptidelabs.example?subject=Order%20${encodeURIComponent(order.reference)}">support@standardpeptidelabs.example</a> with ${escapeHtml(order.reference)} in the subject line. Acknowledgement within one business day.<br><a style="text-decoration:underline;text-underline-offset:2px" href="${path("policies.html")}#fulfillment">Fulfillment and resolution policy</a></p>
              </div>
            </div>
          </div>
  ...
  function accountPage() {
    ...
    const orderRows = orders
      .map((order) => {
        const itemCount = order.items.reduce(
          (total, line) => total + Math.max(1, Number.parseInt(line.quantity, 10) || 1),
          0,
        );
        const placedDate = new Date(order.placedAt || "");
        const placed = Number.isNaN(placedDate.getTime())
          ? order.placedLabel || "Date unavailable"
        ...
        return `
          <article class="account-order">
            <div>
              <p class="eyebrow">Demo order</p>
              <h3>${escapeHtml(order.reference)}</h3>
              <p>${escapeHtml(placed)} · ${itemCount} ${itemCount === 1 ? "item" : "items"} · ${escapeHtml(order.firstName || "")} ${escapeHtml(order.lastName || "")}</p>
              <span>${escapeHtml(order.payment?.processor || DEMO_PROCESSOR)} · ${escapeHtml(order.payment?.brand || "Card")} ending in ${escapeHtml(order.payment?.last4 || "••••")}</span>
            </div>
            <div class="account-order-meta">
              <strong>${money.format(Number(order.totals.total) || 0)}</strong>
              <button class="button button-secondary button-small" type="button" data-open-order="${escapeHtml(order.reference)}">View receipt</button>
            </div>
          </article>
        `;
      })
    ...
    return `
      <section class="page-hero is-compact">
        <div class="shell">
          <span class="sample-watermark">Browser-local demo account</span>
          <p class="eyebrow">Account</p>
          <h1>Your demo orders, on this device.</h1>
          <p>There is no login or customer account. This page reads the cart and order history saved by this browser only.</p>
        </div>
      </section>
      <section class="section">
        <div class="shell">
          <div class="section-head">
            <div>
              ...
              <h2>${orders.length ? `${orders.length} saved demo ${orders.length === 1 ? "order" : "orders"}` : "No saved demo orders"}</h2>
            </div>
            <p>Up to ten orders are retained locally. Raw sandbox card number, expiry, and CVC are never included.</p>
          </div>
          ${
            orders.length
              ? `<div class="account-order-list">${orderRows}</div>`
              : `<div class="empty-state"><strong>Your local order history is empty.</strong><p>Complete the demo checkout to create a receipt and see it here.</p><a class="button button-primary button-small" href="${path("catalog.html")}">Start shopping</a></div>`
          }
          <div class="account-actions">
            <a class="button button-primary" href="${path("catalog.html")}">Browse products</a>
            <button class="button button-secondary" type="button" data-reset-demo>Reset all demo data</button>
          </div>
          <p class="step-hint">Reset removes this storefront's cart, checkout draft, recent products, and order history from browser storage.</p>
        </div>
      </section>
    `;
  }
  ...
  function saveCart() {
    try {
      ...
      showToast("Cart updated for this page, but browser persistence is unavailable.");
    }
    renderCart();
    window.dispatchEvent(
      new CustomEvent("standardpeptidelabs:cart-updated", {
        detail: { itemCount: cartCount(), subtotal: cartTotal() },
      }),
    );
  }
  ...
  function sanitizeCheckoutDraft(value) {
    const draft = value && typeof value === "object" && !Array.isArray(value) ? { ...value } : {};
    delete draft.cardNumber;
    delete draft.cardExpiry;
    delete draft.cardCvc;
    delete draft.payment;
    delete draft.paymentAuthorization;
    return draft;
  }
  ...
  function writeCheckoutDraft(draft) {
    try {
      localStorage.setItem(CHECKOUT_KEY, JSON.stringify(sanitizeCheckoutDraft(draft)));
    } catch (error) {
  ...
  function clearCheckoutDraft() {
    try {
      ...
      /* Cleanup is best-effort; a completed demo order must still reach its receipt. */
    }
  }
    ...
    try {
      ...
      return parsed.filter(
        (order) =>
          ...
          ["subtotal", "discount", "shipping", "tax", "total"].every((key) =>
            Number.isFinite(Number(order.totals[key])),
          ),
      );
    } catch (error) {
      return [];
  ...
  function addBusinessDays(date, count) {
    ...
    let remaining = Math.max(0, Number.parseInt(count, 10) || 0);
    while (remaining > 0) {
  ...
  function priceBreakdown(draft = {}) {
    ...
    const promo = PROMO_CODES[String(draft.promo || "").toUpperCase()] || null;
  ...
  function orderSummaryPanel(breakdown, options = {}) {
    ...
    const items = showItems
      ? cart
          .map((line) => {
            ...
            return `
              <div class="summary-line">
                <img src="${path(item.productSlugs ? "../assets/ovo-set-pair.webp" : "../assets/datum-product-vial.webp")}" alt="" width="48" height="54">
                <div>
    ...
    return `
      <aside class="order-summary" aria-label="Order summary">
        ...
        ${options.cta || ""}
        <p class="summary-note">Prices in USD. This local prototype uses a flat ${(TAX_RATE * 100).toFixed(2)}% demo tax estimate for every address.</p>
        <ul class="assurance-row">
          <li>${icons.lock}Browser local</li>
          <li>${icons.doc}No transmission</li>
          <li>${icons.shield}No card storage</li>
          <li>${icons.doc}Sample records</li>
        </ul>
      </aside>
    `;
  }
      ...
      <section class="drawer-cross" aria-labelledby="drawer-cross-title">
        ...
        ${picks.map((p) => `
          <div class="drawer-cross-row">
            <img src="${path("../assets/datum-product-vial.webp")}" alt="" width="44" height="44">
            <div>
      ...
      .map((line) => {
        ...
        return `
          <article class="cart-item">
            <img src="${path(item.productSlugs ? "../assets/ovo-set-pair.webp" : "../assets/datum-product-vial.webp")}" alt="" width="96" height="108">
            <div>
  ...
  function addToCart(slug, quantity = 1) {
    ...
    const ceiling = cartLineCeiling(slug);
  ...
  function changeCartQuantity(slug, delta) {
    ...
    const requested = line.quantity + delta;
    line.quantity = Math.max(
      0,
      delta > 0 ? Math.min(cartLineCeiling(slug), requested) : requested,
    );
    if (line.quantity === 0) {
  ...
  function initSearch() {
    ...
    if (!searchGlobalBound) {
      searchGlobalBound = true;
      document.addEventListener("click", (event) => {
        if (event.target.closest(".search-wrap")) return;
        const currentInput = document.querySelector("#site-search");
  ...
  function initProduct() {
    ...
    try {
      ...
      /* Browsing remains functional when storage is blocked. */
    }

    const quantity = document.querySelector("#pdp-quantity");
    const quantityButtons = [...document.querySelectorAll("[data-pdp-quantity-change]")];
    const quantityCeiling = cartLineCeiling(product.slug);
    const addControls = [
      ...document.querySelectorAll("[data-pdp-add], [data-mobile-add]"),
    ];
    if (quantityCeiling <= 0) {
      ...
      addControls.forEach((button) => {
        ...
        button.setAttribute(
          "aria-label",
          `${product.name} cannot be added until a contributing cart quantity is lowered`,
        );
      });
      const primaryAdd = document.querySelector("[data-pdp-add]");
      if (primaryAdd) primaryAdd.textContent = "Cart allocation reached";
      const existingNote = document.querySelector("[data-quantity-cap]");
      if (existingNote) {
        existingNote.textContent =
          "Shared inventory is fully allocated by other items or sets in your cart.";
      } else {
        primaryAdd?.closest(".buy-controls")?.insertAdjacentHTML(
          "afterend",
          '<p class="pdp-note" data-quantity-cap>Shared inventory is fully allocated by other items or sets in your cart.</p>',
        );
      }
    }
    const normalizeQuantity = () => {
      ...
      const normalized = Math.max(1, Math.min(quantityCeiling, requested));
      quantity.value = String(normalized);
      quantityButtons.forEach((button) => {
        const delta = Number(button.dataset.pdpQuantityChange);
        button.disabled = delta < 0 ? normalized <= 1 : normalized >= quantityCeiling;
      });
      if (requested > quantityCeiling) {
        showToast(`Demo inventory limits this item to ${quantityCeiling} per order.`);
      }
    };
    quantityButtons.forEach((button) => {
      button.addEventListener("click", () => {
  ...
  function initTestingLookup() {
    ...
    const run = () => {
      ...
      if (!query) {
        results.innerHTML = `<div class="lookup-card"><h3>Enter a product name or code.</h3><p>Examples: Retatrutide, SPL-001, or BPC-157.</p></div>`;
      } else if (matches.length === 0) {
        results.innerHTML = `<div class="lookup-card"><h3>No matching Standard Peptide Labs code.</h3><p>Try a product name, a code from SPL-001 through SPL-010, or browse the full testing table below.</p></div>`;
      } else {
  ...
  function initDemoContact() {
    if (PAGE !== "about") return;
    const form = document.querySelector("[data-demo-contact]");
    ...
    form.addEventListener("submit", (event) => {
      ...
      track("demo_contact_completed", { transmitted: false, stored: false });
    });
  ...
  function initAccount() {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (openOrder) {
        const orders = readOrders();
        const selected = orders.find((order) => order.reference === openOrder.dataset.openOrder);
        if (!selected) {
          showToast("That demo receipt is no longer available.");
          return;
        }
        window.location.href = path(
          `order-confirmation.html?ref=${encodeURIComponent(selected.reference)}`,
        );
      }

      if (reset) {
        const confirmed = window.confirm(
          "Reset this storefront's cart, checkout draft, recent products, and demo order history?",
        );
        if (!confirmed) return;
        try {
          [CART_KEY, RECENT_KEY, ORDER_KEY, CHECKOUT_KEY].forEach((key) => localStorage.removeItem(key));
        } catch {
          /* In-memory state is still reset below when storage is unavailable. */
        }
        cart = [];
  ...
  function initStorageSync() {
    window.addEventListener("storage", (event) => {
      ...
      if (event.key === ORDER_KEY && ["account", "confirmation"].includes(PAGE)) {
        window.location.reload();
      }
    });
  }
  ...
  function bindPromoForm() {
    ...
    form.addEventListener("click", (event) => {
      ...
      if (!input) return;
      input.value = fill.getAttribute("data-fill-promo") || "";
      form.requestSubmit();
    });
  ...
  function initCheckout() {
    ...
    let paymentAuthorizationInFlight = false;
    let orderPlacementInFlight = false;
    ...
    function collect() {
      ...
      delete data.cardCvc;
      delete data.payment;
      delete data.paymentAuthorization;
    ...
    const RULES = {
      ...
      delivery: [],
      payment: [
        ["cardNumber", (v) => passesLuhn(v), "Enter a valid sandbox card number."],
        ["cardExpiry", (v) => validExpiry(v), "Enter a future expiry as MM / YY."],
        ["cardCvc", (v) => /^\d{3,4}$/.test(cardDigits(v)), "Enter a 3 or 4 digit sandbox CVC."],
      ],
    };
    ...
    function paintReview() {
      ...
      holder.innerHTML = [
        ...
        block(
          "Sandbox payment",
          paymentAuthorization && paymentAuthorization.outcome === "approved"
            ? `${escapeHtml(paymentAuthorization.processor)} · Approved for ${money.format(paymentAuthorization.amount)}<br>${escapeHtml(paymentAuthorization.brand)} ending in ${escapeHtml(paymentAuthorization.last4)}<br><span class="lot-nb">${escapeHtml(paymentAuthorization.transactionId)}</span><br>No charge will be made.`
            : "No approved sandbox authorization. Return to payment to continue.",
          "payment",
        ),
      ].join("");
    }

    function paymentMessage(type, message) {
      const selector = type === "error" ? "[data-payment-error]" : "[data-payment-status]";
      const element = form.querySelector(selector);
      if (element) element.textContent = message || "";
    }

    function resetPaymentAuthorization(message = "") {
      const hadAuthorization = Boolean(paymentAuthorization);
      paymentAuthorization = null;
      paymentMessage("error", "");
      paymentMessage("status", message || (hadAuthorization ? "Card details changed. Authorize again before review." : ""));
      const button = form.querySelector('[data-next="payment"]');
      if (button && !paymentAuthorizationInFlight) button.textContent = "Authorize sandbox payment";
    }

    window.addEventListener("standardpeptidelabs:cart-updated", () => {
      if (orderPlacementInFlight) return;
      if (!cart.length || blockedCartLines().length) {
        window.location.href = path("cart.html");
        return;
      }
      const authorizationWasApproved = paymentAuthorization?.outcome === "approved";
      resetPaymentAuthorization(
        authorizationWasApproved
          ? "Cart changed. Authorize the updated total before placing the demo order."
          : "",
      );
      refreshSummary();
      if (authorizationWasApproved || current === "review") show("payment");
    });

    async function authorizePaymentAndReview(button) {
      if (paymentAuthorizationInFlight) return;
      if (!validate("payment")) {
        paymentMessage("status", "");
        paymentMessage("error", "Check the highlighted sandbox payment fields and retry.");
        return;
      }

      const paymentStep = stepEl("payment");
      const number = form.querySelector("#cardNumber")?.value || "";
      const paymentFingerprint = [
        cardDigits(number),
        String(form.querySelector("#cardExpiry")?.value || "").trim(),
        cardDigits(form.querySelector("#cardCvc")?.value || ""),
      ].join("|");
      const authorizationAmount = priceBreakdown(collect()).total;
      paymentAuthorization = null;
      paymentAuthorizationInFlight = true;
      paymentMessage("error", "");
      paymentMessage("status", "Authorizing with DemoPay sandbox. Nothing is being sent...");
      if (paymentStep) paymentStep.setAttribute("aria-busy", "true");
      button.disabled = true;
      button.textContent = "Authorizing in DemoPay...";

      try {
        const result = await authorizeDemoPayment(number);
        const currentPaymentFingerprint = [
          cardDigits(form.querySelector("#cardNumber")?.value || ""),
          String(form.querySelector("#cardExpiry")?.value || "").trim(),
          cardDigits(form.querySelector("#cardCvc")?.value || ""),
        ].join("|");
        if (currentPaymentFingerprint !== paymentFingerprint) {
          paymentMessage(
            "error",
            "Sandbox card details changed during authorization. Review them and retry.",
          );
          button.textContent = "Retry sandbox authorization";
          return;
        }
        if (result.outcome !== "approved") {
          paymentMessage("status", "Sandbox authorization declined. No order was created and no charge was attempted.");
          paymentMessage("error", result.message);
          button.textContent = "Retry sandbox authorization";
          track("sandbox_payment_declined", {
        ...
        if (priceBreakdown(collect()).total !== authorizationAmount) {
          paymentMessage(
            "error",
            "The order total changed during authorization. Review the new total and authorize again.",
          );
          button.textContent = "Retry sandbox authorization";
          return;
        }

        paymentAuthorization = Object.freeze({
          processor: result.processor,
        ...
        paymentMessage(
          "status",
          `${result.processor} approved ${result.brand} ending in ${result.last4}. No charge was made.`,
        );
        persist();
        track("sandbox_payment_approved", {
          processor: result.processor,
        ...
        paymentAuthorizationInFlight = false;
        if (paymentStep) paymentStep.removeAttribute("aria-busy");
        button.disabled = false;
        if (paymentAuthorization) button.textContent = "Re-authorize sandbox payment";
      }
    }

    form.addEventListener("input", (event) => {
      const target = event.target;
      if (target && target.id === "cardNumber") {
        const digits = cardDigits(target.value).slice(0, 19);
        target.value = digits.replace(/(\d{4})(?=\d)/g, "$1 ");
        resetPaymentAuthorization();
      }
      if (target && target.id === "cardExpiry") {
        target.value = String(target.value || "").replace(/[^\d/ ]/g, "").slice(0, 7);
        resetPaymentAuthorization();
      }
      if (target && target.id === "cardCvc") {
        target.value = cardDigits(target.value).slice(0, 4);
        resetPaymentAuthorization();
      }
      persist();
      refreshSummary();
    ...
    form.addEventListener("click", (event) => {
      ...
      if (next) {
        ...
        if (key === "payment") {
          void authorizePaymentAndReview(next);
          return;
        }
      ...
      if (place) void placeOrder();
    });

    function placeOrder() {
      if (orderPlacementInFlight) return;
      const placeButton = form.querySelector("[data-place-order]");
      const orderError = form.querySelector("[data-order-error]");
      if (orderError) orderError.textContent = "";

      if (!cart.length || blockedCartLines().length) {
        if (orderError) {
          orderError.textContent = cart.length
            ? "Your cart changed and now needs attention. Review it before trying again."
            : "Your cart is empty. Add an item before placing a demo order.";
        }
        window.location.href = path("cart.html");
        return;
      }

      if (!paymentAuthorization || paymentAuthorization.outcome !== "approved") {
        show("payment");
        paymentMessage("error", "Authorize an approved DemoPay sandbox card before placing the order.");
        form.querySelector("#cardNumber")?.focus();
        return;
      }

      const liveTotal = priceBreakdown(collect()).total;
      if (paymentAuthorization.amount !== liveTotal) {
        resetPaymentAuthorization(
          "The order total changed after authorization. Authorize the updated total before placing the demo order.",
        );
        show("payment");
        form.querySelector("#cardNumber")?.focus();
        return;
      ...
      if (placeButton) {
        ...
        placeButton.textContent = "Recording demo order...";
      }

      const d = collect();
      const b = priceBreakdown(d);
      const now = new Date();
      const eta = addBusinessDays(
        now,
        b.method.id === "cold" ? 1 : b.method.id === "express" ? 2 : 5,
      );
      const fmt = { month: "short", day: "numeric" };
      const storedPayment = {
        processor: paymentAuthorization.processor,
        transactionId: paymentAuthorization.transactionId,
        outcome: paymentAuthorization.outcome,
        brand: paymentAuthorization.brand,
        last4: paymentAuthorization.last4,
        authorizedAmount: paymentAuthorization.amount,
      };

      const priorOrders = readOrders();
      const existingOrder = priorOrders.find(
        (entry) => entry?.payment?.transactionId === storedPayment.transactionId,
      );
      if (existingOrder) {
        try {
          localStorage.setItem(
            ORDER_KEY,
            JSON.stringify([existingOrder, ...priorOrders.filter((entry) => entry !== existingOrder)].slice(0, 10)),
          );
          cart = [];
          saveCart();
          clearCheckoutDraft();
          window.location.href = path(
            `order-confirmation.html?ref=${encodeURIComponent(existingOrder.reference)}`,
          );
        } catch (error) {
          ...
          if (orderError) orderError.textContent = "The existing demo order could not be reopened from browser storage. Retry once.";
        }
        return;
      }

      const referenceToken = String(storedPayment.transactionId || sandboxTransactionId())
        .replace(/[^a-z0-9]/gi, "")
        .slice(-8)
        .toUpperCase();
      const order = {
        reference: `SPL-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}-${referenceToken}`,
      ...
      try {
        ...
        if (orderError) {
          orderError.textContent = "The demo order could not be saved in this browser. Nothing was charged. Retry once.";
        }
        return;
      }
      ...
      track("order_placed", {
        ...
        processor: storedPayment.processor,
        transaction_id: storedPayment.transactionId,
        outcome: storedPayment.outcome,
      });
      window.location.href = path(
        `order-confirmation.html?ref=${encodeURIComponent(order.reference)}`,
      );
    }
    ...
    if (promoForm) {
      promoForm.addEventListener("submit", (event) => {
        ...
        if (!code) {
          ...
          feedback.textContent = "";
          feedback.classList.remove("is-error");
          if (authorizationWasApproved) {
            resetPaymentAuthorization(
              "Discount changed. Authorize the updated total before placing the demo order.",
            );
            show("payment");
          }
          refreshSummary();
