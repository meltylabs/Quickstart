...
test('portal renders real-data dashboard without empty canvas', async ({ page }) => {
  ...
  await expect(page.getByText('CODEX spatial proteomic imaging').first()).toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: 'Methods Parity' })).toBeVisible();
  await expect(page.getByText('Used directly by portal')).toBeVisible();
  await expect(page.getByText('Synced benchmark receipt')).toBeVisible();
  await expect(page.getByText('Upstream/source artifact')).toBeVisible();
  await expect(page.getByText('Out of scope for this portal')).toBeVisible();
  await expect(page.getByText('CellChat')).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Metric Definitions' })).toBeVisible();
  await expect(page.getByText(/Transfer stays below the same-donor floor/).first()).toBeVisible();
